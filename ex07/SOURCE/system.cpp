...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
	...
	int s_up = +1;	// Starting spin value (up)
	// Compute energy difference for assigning spin up
	double delta_E = 2.0 * s_up * (_J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H);
	// Compute conditional probability for spin up
	double p_up = 1.0 / (1.0 + exp(-_beta * delta_E)); 
	if (_rnd.Rannyu() < p_up)
	  _particle(i).setspin(+1);	// Assign spin up with probability p_up
	else
	  _particle(i).setspin(-1);	// Assign spin down with probability 1-p_up
  } else {           // M(RT)^2
...
double T_check = (2.0 / 3.0) * k_check / _npart;

std::cout << "[CHECK] Energia cinetica iniziale = " << k_check << std::endl;
std::cout << "[CHECK] Temperatura da energia cinetica = " << T_check << std::endl;
  return;
}
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
		// TO BE FIXED IN EXERCISE 7: DONE!
        _vtail = (8.0*M_PI*_rho) * (1.0/9.0 * 1.0/pow(_r_cut, 9) - 1.0/3.0 * 1.0/pow(_r_cut, 3)); 
        ...
        index_property++;
		// TO BE FIXED IN EXERCISE 7: DONE!
        _ptail = (32.0*M_PI*_rho) * (1.0/9.0 * 1.0/pow(_r_cut, 9) - 1.0/6.0 * 1.0/pow(_r_cut, 3)); 
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 5.0 * sqrt(3.0 * _temp)/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4: DONE!
        _measure_pofv = true;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        dr = sqrt( dot(distance,distance) );
        // GOFR ... TO BE FIXED IN EXERCISE 7: DONE!
		if (_measure_gofr) {
  ...
  // POFV ... TO BE FIXED IN EXERCISE 4: DONE!
  if (_measure_pofv) {
    for (int i=0; i<_npart; i++) {
        double vmod = sqrt(dot(_particle(i).getvelocity(), _particle(i).getvelocity()));
        int bin = int(vmod / _bin_size_v);
        if (bin < _n_bins_v) {
  ...
  if (_measure_pressure) {
	  ...
	  else _measurement[_index_pressure] = _rho * _temp + (_ptail*_npart + 48.0*virial/3.0)/_volume;
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6: DONE!
  if (_measure_magnet) {
...
// TO BE FIXED IN EXERCISE 6: DONE!
  if (_measure_cv and _measure_tenergy) {
    _measurement(_index_cv) = tenergy_temp*tenergy_temp * double(_npart);
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6: DONE!
...
void System :: averages(int blk){
  ...
  if (_measure_gofr){
    ...
    for (int i=0; i<_n_bins; i++) {
      ...
      double deltaV = (4.0/3.0) * M_PI * (pow(r+0.5*_bin_size, 3) - pow(r-0.5*_bin_size, 3));
  ...
  // POFV //////////////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 4: DONE!
  if (_measure_pofv) {
  ...
  if (_measure_cv and _measure_tenergy) {
    ...
    average = _beta*_beta * (_average(_index_cv) - _average(_index_tenergy)*_average(_index_tenergy)*double(_npart));
    static double sum_cv = 0.0, sum_cv2 = 0.0;  
    sum_cv += average;
